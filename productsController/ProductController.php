...
class ProductController extends Controller
{
    ...
    public function create()
    {
        ...
        if (categoryCount() <= 0) {
            Alert::info('Hi', translate('Please create category first'));
            return redirect()->route('categories.index');
        }

        if (brandCount() <= 0) {
            Alert::info('Hi', translate('Please create brand first'));
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            if (vendorActive()){
                ...
                // affiliate_site_link
                $affiliate_site_link  = new AffiliateProductLink;
                $affiliate_site_link->product_id = $pro->id;
                $affiliate_site_link->site = $request->site;
                $affiliate_site_link->link = $request->link;
                $affiliate_site_link->clicked = 0;
                $affiliate_site_link->save();
                // affiliate_site_link::END
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            if (vendorActive()){
                ...
                // affiliate_site_link
                $affiliate_product_link = AffiliateProductLink::where('product_id', $pro->id)->first();
                
                if ($affiliate_product_link) {
                    $affiliate_product_link->site = $request->site;
                    $affiliate_product_link->link = $request->link;
                    $affiliate_product_link->save();
                }else {
                    $affiliate_site_link  = new AffiliateProductLink;
                    $affiliate_site_link->product_id = $pro->id;
                    $affiliate_site_link->site = $request->site;
